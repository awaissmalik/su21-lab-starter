...
mapLoop:
    lw t1,0(s0)  #add t1,s0,x0(MISTAKE)::load the address of the array of current node into t1
    ...
    lw a0, 8(s0) #la a0, 8(s0)(MISTAKE)::load the address of the next node into a0
    addi a1,s1,0 #lw a1,0(s0)(MISTAKE)::put the address of the function back into a1 to prepare for the recursion
