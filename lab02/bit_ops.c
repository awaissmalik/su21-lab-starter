...
int main() {
    ...
    unsigned x = 42; // Or any other value
    unsigned n = 2;  // Or any other bit position
    unsigned v = 1;  // Or 0 to clear the bit

    printf("Original value: %u\n", x);

    // Test get_bit
    printf("Bit at position %u: %u\n", n, get_bit(x, n));

    // Test set_bit
    set_bit(&x, n, v);
    printf("After setting bit %u to %u: %u\n", n, v, x);

    // Test flip_bit
    flip_bit(&x, n);
    printf("After flipping bit %u: %u\n", n, x);
